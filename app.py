...
arquivo_a = st.file_uploader("📎 Suba a Planilha A (Data SK)", type=["csv", "xls", "xlsx"])
arquivo_b = st.file_uploader("📎 Suba a Planilha B (Emissões)", type=["csv", "xls", "xlsx"])
...
def ler_arquivo(arquivo):
    ...
    if nome.endswith(".csv"):
        ...
        if ';' in primeira_linha:
            ...
            sep = ','
        try:
            df = pd.read_csv(arquivo, sep=sep, on_bad_lines='skip', encoding='utf-8')
        except UnicodeDecodeError:
            arquivo.seek(0)
...
if arquivo_a and arquivo_b:
    try:
        ...
        if len(df_a.columns) < 6:
            ...
            col_l_valores = df_final.iloc[:, 11].astype(str).str.strip()
            df_final.iloc[:, 1] = np.where(col_l_valores == 'General', 'INTL', 'DOM')
